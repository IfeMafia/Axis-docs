...
The `ruleEvaluated` field will contain one of the following values:

| Rule name              | What it checks                                                          |
| ---------------------- | ----------------------------------------------------------------------- |
| `wallet_active`        | Whether the wallet is active and not suspended                          |
| `spend_limit_per_tx`   | Whether the transaction amount is within the per-transaction spend limit |
| `merchant_allowlist`   | Whether the merchant is on the wallet's approved merchant list          |
| `balance_check`        | Whether the wallet has sufficient balance to cover the transaction      |
| `daily_spend_limit`    | Whether the transaction would exceed the wallet's daily spend cap       |
| `policy_check`         | Whether the transaction satisfies any additional agent policy rules     |
...
```json
{
  ...
  "data": [
    ...
    },
    {
      "id": "ae-uuid-0002",
      "transactionId": "tx-uuid-blocked-001",
      "ruleEvaluated": "spend_limit_per_tx",
      "passed": true,
      "detail": null,
    ...
    },
    {
      "id": "ae-uuid-0003",
      "transactionId": "tx-uuid-blocked-001",
      "ruleEvaluated": "merchant_allowlist",
      "passed": false,
